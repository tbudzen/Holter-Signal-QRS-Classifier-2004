 ...
 */
	...
	{		
		...
		SigmoidNeuralNet(int iSize, int hSize, int sSize) 
		{
			inputSize = iSize;
			hiddenLayer = new SigmoidLayer(hSize, iSize);	
		...
		void propagate(double [] i_vector, double [] e_vector, double gamma) // (given vector, expected vector)
        {
            ...
            {
				n = outputLayer.neurons[i];
               	n.value_d = (i_vector[i] - e_vector[i]) * n.value_d;
            }

	        // Hidden layer - computation of dValue

            for(i = 0; i < hiddenLayer.size; i++)
            {
                    n = hiddenLayer.neurons[i];
                    n.value_d = 0.0;
                                        
					for(j = 0; j < outputLayer.size; j++)
                    {
						n2 = outputLayer.neurons[j];
                        n.dw += n.value_d * n2.dw * n2.weights[j];
                    }                                	
            }
		...
		void load(String fName)
		{
			...
			try
			{
				...
				try
				{
					...
					hiddenLayer = new SigmoidLayer(Integer.parseInt(br.readLine()), inputSize);
					for(i = 0; i < hiddenLayer.size; i++)
					{
						for(j = 0; j < inputSize; j++)
						{
